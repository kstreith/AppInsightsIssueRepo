...
namespace TestAppInsightsPerformance
{
    public class DataClient
    {
        ...
        public async Task<List<WorkLocation>> GetTestData2()
        {
            var resp = await _httpClient.GetAsync("https://appinsightstestdata.blob.core.windows.net/testdata/testlist2.json");
            resp.EnsureSuccessStatusCode();
            var stringContent = await resp.Content.ReadAsStringAsync();
            var content = JsonConvert.DeserializeObject<List<WorkLocation>>(stringContent);
            return content;
